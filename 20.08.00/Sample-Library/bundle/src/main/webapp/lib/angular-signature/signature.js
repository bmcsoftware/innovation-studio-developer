...
angular.module('signature').directive('signaturePad', ['$interval', '$timeout', '$window',
  function ($interval, $timeout, $window) {
    ...
    return {
      restrict: 'EA',
      replace: true,
      template: '<div class="signature" style="width: 100%; max-width:{{width}}px; height: 100%; max-height:{{height}}px;" ng-style="customCss"><canvas style="display: block; margin: 0 auto;" ng-mouseup="onMouseup()" ng-mousedown="notifyDrawing({ drawing: true })"></canvas></div>',
      scope: {
        accept: '=?',
        clear: '=?',
        disabled: '=?',
      ...
      link: function (scope, element, attrs) {
        ...
        // LMA:: BEGIN
        // We have to listen to the width / height since they can be defined by interpolation inside the calling directive / controller.
        scope.customCss = {};
        ...
        scope.$watchGroup([
          ...
          width = scope.width ? parseInt(scope.width, 10) : width;
          height = scope.height ? parseInt(scope.height, 10) : height;

          if (scope.containerCss) {
            try {
              scope.customCss = JSON.parse(scope.containerCss);
            } catch (e) {
              console.debug('Error parsing custom CSS.');
              console.debug(e);
            }
          }
